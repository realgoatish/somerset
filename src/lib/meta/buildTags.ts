...
const buildTags = (config: BuildTagsParams) => {
  ...
  if (config.openGraph) {
    ...
    if (config.openGraph.siteName || config.openGraph.siteName) {
      tagsToRender.push(
        `<meta
          key="og:site_name"
          property="og:site_name"
          content="${config.openGraph.siteName || config.openGraph.siteName}"
        />`,
      );
    }
  }
    ...
    config.additionalMetaTags.forEach(({ keyOverride, ...tag }) => {
      tagsToRender.push(
        `<meta
          ...
          ${ Object.entries(tag).map(([key, value]) => `${key}="${value}"`).join(' ') }
        />`,
      );
    });
  }
  ...
  if (config.additionalLinkTags?.length) {
    config.additionalLinkTags.forEach(tag => {
      tagsToRender.push(
        `<link key="link${tag.keyOverride ?? tag.href}${tag.rel}" ${ Object.entries(tag).map(([key, value]) => `${key}="${value}"`).join(' ') } />`,
      );
    });
  }

  // console.log(tagsToRender.join(''))
